...
export function ShippingListPage() {
  ...
  const columns = useMemo<ColumnDef<ShippingMethodRow, any>[]>(
    () => [
      {
        id: "name",
        accessorKey: "name",
        header: ({ column }: { column: Column<ShippingMethodRow> }) => (
          <div className="flex items-center gap-1">
            {t("admin.labels.shippingName")}
            <DataTableSorter column={column} />
          </div>
        ),
        cell: ({ row }: { row: { original: ShippingMethodRow } }) => (
      ...
      },
      {
        ...
        header: t("admin.labels.shippingDeliveryTime"),
      ...
      },
      {
        ...
        header: t("admin.labels.shippingFreeThreshold"),
        cell: ({ getValue }: { getValue: () => unknown }) => {
          const val = getValue() as number | null;
          return val != null ? (
            <span className="text-sm">{formatPrice(Number(val))}</span>
          ) : (
      ...
      },
      {
        ...
        header: t("admin.labels.shippingDefault"),
        cell: ({ getValue }: { getValue: () => unknown }) =>
          getValue() ? (
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold">
          {t("admin.labels.shippingMethods")}
        </h1>
        <Button onClick={() => navigate("/admin/shipping/new")}>
          <Plus className="h-4 w-4 mr-2" /> {t("admin.labels.newShipping")}
        </Button>
      </div>
      ...
      <div className="flex flex-wrap gap-3 items-end">
        <div className="w-60">
          <Input
            placeholder={t("admin.labels.shippingSearchName")}
            value={search}
      ...
      <DataTable
        table={table}
        onRowClick={(row) => navigate(`/admin/shipping/${row.id}`)}
      />
    </div>
  );
}
