...
export function FrameListPage() {
  ...
  const columns = useMemo<ColumnDef<PictureFrameRow, any>[]>(
    () => [
      {
        id: "name",
        accessorKey: "name",
        header: ({ column }: { column: Column<PictureFrameRow> }) => (
          <div className="flex items-center gap-1">
            {t("admin.labels.frameName")}
            <DataTableSorter column={column} />
          </div>
        ),
        cell: ({ row }: { row: { original: PictureFrameRow } }) => (
      ...
      },
      {
        ...
        header: t("admin.labels.frameMaterial"),
      ...
      },
      {
        ...
        header: t("admin.labels.frameDefault"),
        cell: ({ getValue }: { getValue: () => unknown }) =>
          getValue() ? (
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold">{t("admin.labels.frames")}</h1>
        <Button onClick={() => navigate("/admin/frames/new")}>
          <Plus className="h-4 w-4 mr-2" /> {t("admin.labels.newFrame")}
        </Button>
      </div>
      ...
      <div className="flex flex-wrap gap-3 items-end">
        <div className="w-60">
          <Input
            placeholder={t("admin.labels.frameSearchName")}
            value={search}
      ...
      <DataTable
        table={table}
        onRowClick={(row) => navigate(`/admin/frames/${row.id}`)}
      />
    </div>
  );
}
