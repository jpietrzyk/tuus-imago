...
/**
 * Chainable stand-in for a PostgREST query builder. Aggregates/exports now page
 * via `.order().range()`, so the terminal `.range()` resolves the fixture.
 */
function paged(result: { data?: unknown; error?: unknown }) {
...
describe("admin-api handler", () => {
  ...
  describe("meta column validation", () => {
    it("rejects an unknown filter column", async () => {
      mockFetchForAuth({ id: "admin-1", email: "admin@test.com" });
      ...
      const response = await handler({
        ...
        body: JSON.stringify({
          resource: "orders",
          meta: { filters: [{ field: "password", operator: "eq", value: "x" }] },
        }),
      });
    ...
    it("rejects an unknown select field", async () => {
      ...
      const response = await handler({
        ...
        body: JSON.stringify({
          resource: "orders",
          meta: { select: "id, secret_column" },
        }),
      });

      expect(response.statusCode).toBe(400);
      expect(readBody(response).error).toContain("Invalid select fields");
    });

    it("rejects an unknown groupBy column", async () => {
      mockFetchForAuth({ id: "admin-1", email: "admin@test.com" });
      setupClient({ profiles: makeAdminAuthCheck() });
      ...
      const response = await handler({
        ...
        body: JSON.stringify({
          resource: "orders",
          meta: { aggregateFunction: "count", groupBy: "customer_email" },
        }),
      });

      expect(response.statusCode).toBe(400);
      expect(readBody(response).error).toContain("Invalid group column");
    });

    it("rejects an unknown sum field", async () => {
      mockFetchForAuth({ id: "admin-1", email: "admin@test.com" });
      setupClient({ profiles: makeAdminAuthCheck() });
      ...
      const response = await handler({
        ...
        body: JSON.stringify({
          resource: "orders",
          meta: { aggregateFunction: "sum", aggregate: "customer_email" },
        }),
      });
    ...
    it("returns stats for a single partner including referral orders", async () => {
      ...
      fromSpy.mockImplementation((table: string) => {
        switch (table) {
          ...
          case "coupons":
            return paged({ data: couponsData, error: null });
          case "coupon_usages":
            return paged({ data: usagesData, error: null });
          case "orders": {
            ordersCallCount++;
            if (ordersCallCount === 1) {
              return paged({ data: refOrdersData, error: null });
            }
            return paged({ data: allOrdersData, error: null });
          }
          case "partner_refs":
            return paged({ data: refsData, error: null });
          case "referral_events":
            return paged({ data: [], error: null });
          default:
            throw new Error(`Unexpected table: ${table}`);
        }
      });
      ...
      const response = await handler({
        ...
        body: JSON.stringify({
          ...
          meta: {
            ...
            groupBy: "11111111-1111-1111-1111-111111111111",
          },
        }),
      });
    ...
    it("returns stats for all partners with cross-referenced coupons, refs, orders, revenue", async () => {
      ...
      fromSpy.mockImplementation((table: string) => {
        switch (table) {
          ...
          case "partners":
            return paged({ data: partnersData, error: null });
          case "coupons":
            return paged({ data: allCouponsData, error: null });
          case "partner_refs":
            return paged({ data: allRefsData, error: null });
          case "coupon_usages":
            return paged({ data: usagesData, error: null });
          case "orders": {
            ordersCallCount++;
  ...
  describe("export CSV", () => {
    it("returns CSV response with correct headers (partners)", async () => {
      ...
      setupClient({ profiles: authCheck, partners: paged({ data: partnersData, error: null }) });
    ...
    it("exports orders CSV with field escaping", async () => {
      ...
      client.from.mockImplementation((table: string) => {
        switch (table) {
          ...
          case "orders":
            return paged({ data: ordersData, error: null });
          default:
            throw new Error(`Unexpected table: ${table}`);
        }
      });
    ...
    it("exports customers CSV through the export-only resource", async () => {
      mockFetchForAuth({ id: "admin-1", email: "admin@test.com" });
      const authCheck = makeAdminAuthCheck();
      ...
      expect(response.statusCode).toBe(200);
      expect(response.headers["Content-Type"]).toBe("text/csv");
      expect(response.headers["Content-Disposition"]).toContain("customers-export-");
      expect(response.body).toContain("a@t.com");
    });
    ...
    it("applies filters to orders export", async () => {
      ...
      const builder: Record<string, unknown> = {};
      const self = () => builder;
      eqMock.mockImplementation(self);
      builder.eq = eqMock;
      builder.order = vi.fn(self);
      builder.range = vi.fn().mockResolvedValue({ data: ordersData, error: null });
      const selectMock = vi.fn(self);
  ...
  describe("picture_frames", () => {
    ...
    it("returns 400 for unknown resource", async () => {
      ...
      const response = await handler({
        httpMethod: "GET",
        headers: { authorization: "Bearer valid-token" },
        queryStringParameters: { resource: "picture_framez" },
      });
    ...
    it("clears the default flag on other frames when setting a new default", async () => {
      mockFetchForAuth({ id: "admin-1", email: "admin@test.com" });
      ...
      const single = vi.fn().mockResolvedValue({
        data: { id: "frame-2", name: "Black Metal", is_default: true },
        error: null,
      });
      const select = vi.fn().mockReturnValue({ single });
      const eq = vi.fn().mockReturnValue({ select });
      ...
      client.from.mockImplementation((table: string) => {
        if (table === "picture_frames") {
          framesCallCount++;
          if (framesCallCount === 1) {
      ...
      expect(unsetNeq).toHaveBeenCalledWith("id", "frame-2");
      expect(update).toHaveBeenCalledWith({ is_default: true });
    });

    it("clears is_default when deactivating a frame", async () => {
      mockFetchForAuth({ id: "admin-1", email: "admin@test.com" });

      const single = vi.fn().mockResolvedValue({
        data: { id: "frame-1", name: "Oak Classic", is_active: false, is_default: false },
        error: null,
      });
      const select = vi.fn().mockReturnValue({ single });
      const eq = vi.fn().mockReturnValue({ select });
      ...
      setupClient({ profiles: makeProfileAuth(), picture_frames: { update } });
    ...
    it("creates a frame and clears existing defaults when created as default", async () => {
      mockFetchForAuth({ id: "admin-1", email: "admin@test.com" });
      ...
      const single = vi.fn().mockResolvedValue({
        data: { id: "frame-new", name: "Walnut", is_default: true },
        error: null,
      });
      const select = vi.fn().mockReturnValue({ single });
      const insert = vi.fn().mockReturnValue({ select });
      ...
      const response = await handler({
        ...
        body: JSON.stringify({
          resource: "picture_frames",
          data: { name: "Walnut", price: 59, is_active: true, is_default: true },
        }),
      });
      ...
      expect(insert).toHaveBeenCalledWith(
        expect.objectContaining({ name: "Walnut", is_default: true }),
      );
    });

    it("forces is_default false when creating an inactive frame", async () => {
      mockFetchForAuth({ id: "admin-1", email: "admin@test.com" });

      const single = vi.fn().mockResolvedValue({
        data: { id: "frame-new", name: "Draft", is_active: false, is_default: false },
        error: null,
      });
      const select = vi.fn().mockReturnValue({ single });
      const insert = vi.fn().mockReturnValue({ select });

      setupClient({ profiles: makeProfileAuth(), picture_frames: { insert } });
